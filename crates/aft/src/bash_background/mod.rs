...
pub fn spawn(request_id: &str, command: &str, ctx: &AppContext) -> Response {
    ...
    let workdir = ctx.config().project_root.clone().unwrap_or_else(|| {
    ...
    match ctx
        .bash_background()
        .spawn(command, workdir, storage_dir, max_running)
    {
