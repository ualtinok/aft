...
pub fn extract_calls_full(
    ...
    let mut results = Vec::new();
    let call_kinds = call_node_kinds(lang);
    collect_calls_full(root, source, byte_start, byte_end, &call_kinds, &mut results);
    results
}
...
fn collect_calls_full(
    ...
    if call_kinds.contains(&node.kind()) && node_start >= byte_start && node_end <= byte_end {
        if let (Some(full), Some(short)) = (extract_full_callee(&node, source), extract_callee_name(&node, source)) {
    ...
    if cursor.goto_first_child() {
        loop {
            collect_calls_full(cursor.node(), source, byte_start, byte_end, call_kinds, results);
            if !cursor.goto_next_sibling() {
                break;
            }
        }
